...
sns.pairplot(data=diab, hue='Outcome')
plt.show()
...
Y = diab['Outcome']
model.fit(X,Y)
series = pd.Series(model.feature_importances_, index=X.columns).sort_index(ascending=False)
print(series)
...
linear_svc = svm.SVC(kernel='linear', C=0.1,probability=True)
radial_svc = svm.SVC(kernel='rbf', C=0.1,gamma=10,probability=True)
lr = LogisticRegression(C=0.1)

from sklearn.ensemble import VotingClassifier
ensemble_lin_rbf = VotingClassifier(estimators=[('Linear_svm', linear_svc), ('Radial_svm', radial_svc)],
...
ensemble_rad_lr=VotingClassifier(estimators=[('Radial_svm', radial_svc), ('Logistic Regression', lr)],
...
ensemble_rad_lr_lin=VotingClassifier(estimators=[('Radial_svm', radial_svc), ('Logistic Regression', lr),('Linear_svm',linear_svc)],
